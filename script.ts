...
(function () {
    ...
    if (campoNomeCidade && botaoBusca) {
        ...
        campoNomeCidade?.addEventListener(
            "keyup",
            function (e) {
...
const WEATHER_CODES = new Map<number, TempoInfo>([
...
// ---------------------------------------------------------------------------------------
function getWeatherCodeDesc( code : number ) : TempoInfo | undefined {
    return WEATHER_CODES.get(code);
}

// ---------------------------------------------------------------------------------------
function getUVString(valorIndice : number) : string {
    if (valorIndice < 3) {
        return "Baixo";
...
// ---------------------------------------------------------------------------------------
function buscaCoordenadas(nomeCidade : string) {
    // debugger;
    if (nomeCidade.length > 0) {
        //TODO: validate input

        const param : string = encodeURIComponent(nomeCidade);
        const url : string = "https://nominatim.openstreetmap.org/search?q=" + param + "&format=geojson";
...
// ---------------------------------------------------------------------------------------
function requestLoadHandler(this: XMLHttpRequest ) {
    if (this.readyState == 4 && this.status == 200) {
        ...
        if (geoJSON && len > 0) {
            ...
            let nomeCidadeElement = document.querySelector<HTMLElement>("#nome-cidade");
...
// ---------------------------------------------------------------------------------------
function requestErrorHandler( this : XMLHttpRequest ) {
    console.error(this.response);
}

// ---------------------------------------------------------------------------------------
function weatherRequestLoadHandler(this: XMLHttpRequest ) {
    if (this.readyState == 4 && this.status == 200) {
        ...
        const previsaoBody = document.querySelector<HTMLElement>(".semana-grid");
        let content = "";
            ...
            content += `
                <div class='semana-grid-item'>
                    ...
                    <div class='semana-grid-item-right'>
                        <p class='cartao-titulo'>${weatherCode?.desc}</p>
        ...
        const cidadeElement = document.querySelector<HTMLElement>("#nome-cidade");
        if( cidadeElement ) {
...
// ---------------------------------------------------------------------------------------
function weatherRequestErrorHandler(this: XMLHttpRequest) {
    console.error(this.response);
}